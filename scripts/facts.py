...
#
# Collected for /for/ecommerce/, and the result cut against the page that was
# going to be written. Large retailers are mostly clean: no broken Product
# schema, no thin product pages. The page says so, because a survey that
# disagrees with the pitch is the one worth publishing.
#
# Generated by scripts/collect_ecommerce.py.
...
def ecom_measured() -> str:
    return ecom()["measured"]


def ecom_shops() -> int:
    return ecom()["shops_measured"]


def ecom_attempted() -> int:
    return ecom()["shops_attempted"]


def ecom_unreachable() -> int:
    return ecom()["shops_unreachable"]


def ecom_page_cap() -> int:
    return ecom()["page_cap"]


def ecom_median_score() -> float:
    return ecom()["median_score"]


def ecom_best_score() -> float:
    return ecom()["best_score"]


def ecom_worst_score() -> float:
    return ecom()["worst_score"]


def ecom_schema_problems() -> int:
    return ecom()["with_schema_problem"]


def ecom_thin() -> int:
    return ecom()["with_thin_pages"]


def ecom_rating_unconfirmed() -> int:
    return ecom()["with_rating_unconfirmed"]


def ecom_indexing() -> int:
    return ecom()["with_indexing_problem"]


# -- Google's own published crawler address ranges ---------------------------
#
# The evidence for /learn/log-file-analysis/'s central claim: a user-agent is a
# header anybody can send, so identifying Googlebot means either checking these
# lists or doing the reverse-DNS dance. Counted from Google's primary source,
# which carries its own creationTime because the lists move.
#
# The spot check is a smoke test, not a rate. Six addresses must never be
# published as a percentage — facts.py exposes the numerator and denominator
# separately so the page has to say both.
#
# Generated by scripts/collect_googlebot.py.
...
def gbot_measured() -> str:
    return googlebot()["measured"]


def gbot_total_prefixes() -> int:
    return googlebot()["total_prefixes"]


def gbot_prefixes() -> int:
    return googlebot()["googlebot_prefixes"]


def gbot_ipv4() -> int:
    return googlebot()["googlebot_ipv4"]


def gbot_ipv6() -> int:
    return googlebot()["googlebot_ipv6"]


def gbot_lists() -> int:
    return len(googlebot()["lists"])


def gbot_spot_size() -> int:
    return googlebot()["spot_check_size"]


def gbot_spot_verified() -> int:
    return googlebot()["spot_check_verified"]


# -- the download, measured from the artifact actually published -------------
#
# `DMG_SIZE` was the string "17 MB" typed into render.py against an 18.3 MB
# file. Decimal MB deliberately: a browser's download panel and the Finder both
# report decimal, so the binary MiB figure would understate what the user sees.
#
# Generated by scripts/collect_updater.py, which runs after every build.
