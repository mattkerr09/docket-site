...
Scout audited scoutseo.app and raised content.missing_trust_pages: no About, no
...
footer on all 25 pages advertised hello@scoutseo.app, and scoutseo.app has no
...
def about() -> Path:
    body = f"""
<p class="lede">Scout is built by Matt Kerr, one person, in the UK. It is a Mac app that
...
<p>Scout is the other shape. The audit engine is Python with no third-party dependencies at
...
introduce itself as any crawler you like. That is how Scout can tell you what your CDN
returns to <code>GPTBot</code> specifically, rather than what your robots.txt claims it
should.</p>

<p>It is also why the awkward findings are possible. Scout will tell you your AI opt-out is
...
<h2>What Scout is not good at</h2>
...
spent years and a great deal of money building those, and Scout has no answer to them.</li>
...
you sort them; Scout is built to read a site closely, not to survey one at that scale.</li>
<li><strong>The app's source is not public.</strong> The website and its datasets are; the
application is not. If your policy requires reading the code, Scout will not pass it.</li>
...
<pre><code>codesign -dv --verbose=4 /Volumes/Scout/Scout.app
spctl -a -t open --context context:primary-signature -v Scout-0.1.0-arm64.dmg</code></pre>
...
<p>Tell us, with the URL. Scout has shipped false positives — the byte-cap check once
    ...
    return render(
        ...
        faq=[
            ...
            ("Does Scout send my site data anywhere?",
             "No. The crawl runs on your Mac and results are stored in ~/.scout/ as plain "
             "JSON. Two optional connectors do reach outside: the knowledge refresh fetches "
             "one public file from scoutseo.app and tells it nothing about what you are "
...
<a href="{ISSUES}">github.com/mattkerr09/scout-site/issues</a>. It is read by the person who
...
<code>hello@scoutseo.app</code>. That address could not receive mail, and never had.</p>

<p>Delivering mail to a domain means looking up its MX record. <code>scoutseo.app</code> has
...
Scout is built to catch and did not: a channel that is advertised, believed, and dead. It is
...
doing</td></tr>
<tr><td>Ask whether Scout does something</td>
<td>Check <a href="/learn/what-scout-checks/">the full check list</a> first, then
...
documentation says otherwise, a link to it settles the question immediately. Scout's checks
...
report naming a specific URL and check ID gets a specific answer, and that if Scout is wrong
    ...
    return render(
        ...
        faq=[
            ...
            ("Is there a support email address?",
             "No. The site previously listed hello@scoutseo.app, which could not receive "
