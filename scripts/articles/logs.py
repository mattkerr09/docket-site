...
check that Scout's verification accepts addresses drawn from them.
...
Scout does both, and there is a test that fails if the forward lookup is ever removed.</p>

<p>Scout's verification was spot-checked against {F.gbot_spot_size()} addresses taken from
...
<p>Run <code>scout logs access.log --url https://example.com</code> and it crawls the site as
...
<p><strong>Pages you link to that Google did not fetch.</strong> Careful here, and Scout prints
...
than what Scout does: a real interface for exploring the data, saved projects, imports that
handle far more formats, and analysis over time rather than a single comparison. If log
analysis is a regular part of your work, buy it — it is built for that and Scout is not.</p>

<p>What Scout gives you is the one comparison that answers "is Google spending its time on my
...
<h2>What Scout deliberately does not do here</h2>
...
"""
    return render(
        ...
        crumb='<a href="/">Scout</a> / <a href="/learn/">Learn</a> / Log file analysis',
        body=body,
        ...
        faq=[
            ...
            ("Does Scout replace a dedicated log file analyser?",
             ...
             "over time. Scout does one comparison, log against crawl, included in its "
