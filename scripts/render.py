...
# --------------------------------------------------------------------------
STYLE = """<style>
/* Typefaces self-hosted rather than pulled from a CDN. Scout flags
   render-blocking third-party resources in its audits, and shipping one on its
   own site would be the kind of inconsistency it exists to catch. 97 KB total. */
@font-face{font-family:'Clash Display';src:url('/fonts/ClashDisplay-600.woff2') format('woff2');
...
:root{
  ...
  --sans:'General Sans',-apple-system,BlinkMacSystemFont,"Segoe UI",Helvetica,sans-serif;
...
.hero-h1{font-family:var(--display);font-weight:600;
  ...
  font-size:clamp(2.5rem,5.6vw,4.15rem);line-height:1.04;letter-spacing:-.04em;
...
.sec-head h2{font-family:var(--display);font-weight:600;
  font-size:clamp(1.95rem,4.2vw,3.05rem);line-height:1.08;
